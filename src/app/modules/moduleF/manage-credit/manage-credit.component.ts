...
import { CommonListComponent, moneyToNumber } from 'app/shared';
import { Credit,
         CreditTypes,
         CreditService,
...
@Component({
  selector: 'moduleF-manage-credit',
  templateUrl: './manage-credit.component.html',
  styleUrls: ['./manage-credit.component.scss']
})
export class ManageCreditComponent extends CommonListComponent implements OnInit {
  ...
  // tipos de bonus
  public creditType = this.itemFilterList = CreditTypes;
  // referencia a funcao
  ...
  constructor(private creditService: CreditService,
    ...
    //                modulo      pagina
    super(injector, ['moduleF','manageCredit']);
   ...
   */
  public listData(): Credit[] {

    return this._listData(this.list, this.filter, 'entryDate:datetime', 'type:objectRep', 'creditValue', 'user.firstName', 'user.lastName');
  ...
  public find(): void {
    ...
    if ( this.idCFC && (this.initialTimestamp < this.finalTimestamp) ) {
      ...
      this.creditService.loadData(this.idCFC, { initialTimestamp: this.initialTimestamp, finalTimestamp: this.finalTimestamp })
        .subscribe(
          res => {
            this.list = res;
   ...
   */
  public showForm(data: Credit = null): void {
  ...
  public delete(id:number): void {
    ...
    this._delete(id, this.creditService);
